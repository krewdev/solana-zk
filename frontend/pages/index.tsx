...
// Your program ID from environment
...
const VERIFIER_OPTIONS: VerifierOption[] = [
  {
    id: 'verisol-github-repos',
    name: 'VeriSol GitHub Repo Proof',
    description:
      'Current verifier. Proves on-chain that your GitHub account has at least one public repository before minting the developer cATT.',
  },
  {
    id: 'verisol-commit-streak',
    name: 'Commit Streak Attestation (soon)',
    description:
      'Planned verifier that checks sustained GitHub commit activity before issuing an attestation.',
    comingSoon: true,
  },
  {
    id: 'verisol-reviewer-score',
    name: 'Code Review Score (soon)',
    description:
      'Concept verifier that verifies proof of thoughtful PR reviews using ZK circuits and GitHub data snapshots.',
    comingSoon: true,
  },
]
...
const Home: NextPage = () => {
  ...
  const [selectedVerifierId, setSelectedVerifierId] = useState<string>(VERIFIER_OPTIONS[0]?.id ?? '')

  const selectedVerifier = useMemo(
    () => VERIFIER_OPTIONS.find((option) => option.id === selectedVerifierId),
    [selectedVerifierId]
  )

  // Derive Bubblegum Tree Authority PDA from merkle tree
  const DERIVED_TREE_AUTHORITY = useMemo(() => {
    try {
  ...
  const handlePreflight = useCallback(async () => {
    try {
      ...
      const merkleTree = MERKLE_TREE.toBase58()
      const treeAuth = TREE_AUTHORITY.toBase58()
      const bubblegum = BUBBLEGUM_PROGRAM_ID.toBase58()
      const compression = COMPRESSION_PROGRAM_ID.toBase58()
      const noop = LOG_WRAPPER.toBase58()

      const [progAcc, treeAcc, authAcc, bgAcc, compAcc, noopAcc] = await Promise.all([
        connection.getAccountInfo(PROGRAM_ID),
        connection.getAccountInfo(MERKLE_TREE),
        connection.getAccountInfo(TREE_AUTHORITY),
        connection.getAccountInfo(BUBBLEGUM_PROGRAM_ID),
        connection.getAccountInfo(COMPRESSION_PROGRAM_ID),
        connection.getAccountInfo(LOG_WRAPPER),
      ])

      setDebugInfo([
        `RPC: ${rpc}`,
        `Program: ${programId} exists=${!!progAcc}`,
        `MerkleTree: ${merkleTree} exists=${!!treeAcc}`,
        `TreeAuthority (env): ${treeAuth} exists=${!!authAcc}`,
        `TreeAuthority (derived): ${DERIVED_TREE_AUTHORITY?.toBase58() || 'n/a'}`,
        `TreeAuthority match: ${DERIVED_TREE_AUTHORITY ? (DERIVED_TREE_AUTHORITY.toBase58() === treeAuth) : 'n/a'}`,
        `Bubblegum: ${bubblegum} exists=${!!bgAcc}`,
        `Compression: ${compression} exists=${!!compAcc}`,
        `Noop: ${noop} exists=${!!noopAcc}`,
      ].join('\n'))
    } catch (e: any) {
      setDebugInfo(`Preflight error: ${e?.message || e}`)
    }
  }, [connection, DERIVED_TREE_AUTHORITY])
  ...
  const handleVerifierChange = useCallback((event: ChangeEvent<HTMLSelectElement>) => {
    setSelectedVerifierId(event.target.value)
  }, [])
  ...
  console.log('🚀 Starting VeriSol minting process...')
    console.log('Configuration:', {
      programId: PROGRAM_ID.toString(),
      merkleTree: MERKLE_TREE.toString(),
      treeAuthority: TREE_AUTHORITY.toString(),
      bubblegumProgramId: BUBBLEGUM_PROGRAM_ID.toString(),
      compressionProgramId: COMPRESSION_PROGRAM_ID.toString(),
      logWrapper: LOG_WRAPPER.toString(),
      walletConnected: wallet.connected,
      walletAddress: wallet.publicKey?.toString(),
      sessionUser: session?.user?.name,
      verifier: selectedVerifier?.id,
    })
    ...
    if (!selectedVerifier || selectedVerifier.comingSoon) {
      setStatus('Selected verifier is not available yet. Choose an active verifier to mint.')
      return
    }

    setIsLoading(true)
    setStatus(`Checking wallet balance for ${selectedVerifier.name}...`)
    ...
    try {
      ...
      setStatus(`✅ Balance OK: ${solBalance.toFixed(4)} SOL. Preparing ${selectedVerifier.name} inputs...`)
    } catch (balanceError) {
    ...
    try {
      ...
      setStatus(`Found ${publicRepos} public repos. Generating ZK proof for ${selectedVerifier.name}...`)
      ...
      setStatus(`${selectedVerifier.name} proof generated. Preparing transaction...`)
  ...
  setStatus(`Attempting full compressed NFT minting using ${selectedVerifier.name}...`)
        ...
        try {
          const tx = await program.methods
            ...
            .accounts({
              ...
              treeAuthority: (DERIVED_TREE_AUTHORITY ?? TREE_AUTHORITY),
          ...
          console.error('Mint error details:', {
            ...
            logs: mintError.logs || mintError.transactionLogs,
            signature: mintError.signature,
            programErrorStack: mintError.programErrorStack
          })
          ...
          try {
            ...
            console.error('Proof-only verification failed:', fallbackErr)
            setStatus('❌ Proof-only verification failed too. See console for details.')
            throw fallbackErr
          }
        }
        ...
        console.error('Error details:', {
          ...
          logs: txError.logs || txError.transactionLogs,
        ...
        if (txError.message.includes('Simulation failed')) {
          ...
          console.error('Simulation logs:', txError.logs || txError.transactionLogs)
        ...
        } else if (txError.message.includes('program that does not exist')) {
          setStatus('❌ Program not found on blockchain. Check program ID in environment.')
        } else {
  ...
  }, [wallet, session, connection, selectedVerifier, DERIVED_TREE_AUTHORITY])
  ...
  <title>VeriSol - Mint GitHub Developer cATT</title>
      ...
      <div className="mt-6 p-3 border rounded text-sm whitespace-pre-wrap">
        <div className="font-semibold mb-2">Debug</div>
        <div className="mb-2">
          <Button onClick={handlePreflight} variant="secondary">Run Preflight</Button>
        </div>
        {debugInfo || 'Click "Run Preflight" to check accounts and RPC'}
      </div>
        ...
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Choose a verifier from the evolving VeriSol library and issue a zero-knowledge backed credential directly to Solana.
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            {/* Step 3: Choose Attestation */}
            <div className="space-y-2">
              <h3 className="text-sm font-medium">Step 3: Choose Attestation</h3>
              <p className="text-xs text-gray-600">
                Select which verifier to execute. Future releases will let you mint from a full library of developer attestations.
              </p>
              <select
              ...
              >
                {VERIFIER_OPTIONS.map((option) => (
                  <option key={option.id} value={option.id} disabled={option.comingSoon}>
                    {option.name}
                  </option>
                ))}
              </select>
              {selectedVerifier && (
                <div className="rounded-md border border-dashed border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-900">
                  <p className="font-medium">What this verifier checks</p>
                  <p className="mt-1 leading-relaxed">
                    {selectedVerifier.description}
                  </p>
                  {selectedVerifier.comingSoon && (
                    <p className="mt-2 font-semibold text-amber-700">
                      Coming soon — we are prototyping this verifier now.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-medium">Step 4: Generate Proof & Mint cATT</h3>
              <Button
                onClick={handleMint}
                disabled={
                  !wallet.connected ||
                ...
                {isLoading 
                  ...
                  : !wallet.connected 
                    ...
                    : !session
                      ...
                      : !selectedVerifier || selectedVerifier.comingSoon
                        ? 'Select an Active Verifier'
                      : solBalance !== null && solBalance < 0.01
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-xs text-slate-700">
              <h4 className="text-sm font-semibold text-slate-900">What happens behind the scenes</h4>
              <ol className="mt-2 space-y-1 list-decimal list-inside">
                <li>We fetch your GitHub public profile through NextAuth and map it to circuit inputs.</li>
                <li>The snarkjs runtime generates a Groth16 proof in your browser using the selected verifier circuit.</li>
                <li>We send the proof to the VeriSol Anchor program, which verifies it and CPIs into Bubblegum to mint a compressed attestation.</li>
                <li>If Bubblegum minting fails, we still record the proof on-chain so your attestation can be replayed later.</li>
              </ol>
            </div>
              ...
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                ...
                <p className="text-xs text-green-700 mt-1">
                  {txSignature.startsWith('demo_') 
                    ...
                    : status.includes('Compressed NFT minted')
                      ? `Your GitHub activity has been verified with zero-knowledge proofs and minted as a compressed NFT attestation on Solana using ${selectedVerifier?.name ?? 'the selected verifier'}!`
                      : status.includes('proof-only')
                        ? `Your GitHub activity has been cryptographically verified using ${selectedVerifier?.name ?? 'the selected verifier'} and logged on the Solana blockchain!`
                        : `Your GitHub activity has been verified with zero-knowledge proofs via ${selectedVerifier?.name ?? 'the selected verifier'}!`
                  }
                </p>
                {!txSignature.startsWith('demo_') && (
                  <a
